...
def get_distance_btw_violation_by_distance_matrix_api(lat1, lng1, lat2, lng2):
    print("get_distance_btw_violation_by_distance_matrix_api")
...
def retry_upload_dataset(dataset):
    ...
    for st in list_ava_str:
        insert_violations_Street(st['violation'])
    for st in range(len(list_ava_str)):
        vio_list = []
        for vio in list_ava_str[st]['violation']:
            if int(vio['accurate']) >= 80:
                vio_list.append(vio)
        list_ava_str[st]['violation'] = vio_list
...
def upload_dataset(dataset):
    ...
    for i in set_of_street_names:
        ...
        for j in range(len(dataset)):
            if dataset[j]['street_name'] == i:
                ...
                set_of_street_ids.add(int(st_id))
                for mk in list_of_violations:
                    if dataset[j]['violation'] == mk['violationname']:
                        dataset[j]['violation_type_id'] = mk['violationtypeid']
                temp.append(dataset[j])
    ...
    for street in list_ava_str:
        ref_pointa, prev_point_one, prev_point_two, total_violation = get_prev_violations_StreetID(
            street['street_id'])
        ...
        if total_violation == 0:
            ...
            if len(street['violation']) > 1:
                ...
                point_one.append(street['violation'][0])
                for violation in range(1, len(street['violation'])):
                    print(street['street_id'], violation)
                    if get_distance_btw_violation_by_distance_matrix_api(street['violation'][0]['lat'],
                                                                         street['violation'][0]['long'],
                                                                         street['violation'][violation]['lat'],
                                                                         street['violation'][violation]['long']):
                ...
                if len(point_one) > 1:
                    ...
                    if max_distance <= 300:
                        temp = make_responce_of_two_point(str(point_one[max_distance_point_01]['lat']) + "," + str(
                            point_one[max_distance_point_01]['long']),
                                                          str(point_one[max_distance_point_02]['lat']) + "," + str(
                                                              point_one[max_distance_point_02]['long']))
                        for mb in temp:
                            street['side01'].append(mb)
                            ...
                            if dist <= 300:
                                ...
                                if last_point == -1:
                                    ...
                                    if len(got_sec) == 2:
                                        ...
                                        temp = make_responce_of_two_point(
                                            str(got_sec[0]['lat']) + "," + str(got_sec[0]['lng']),
                                            str(got_sec[1]['lat']) + "," + str(got_sec[1]['lng']))
                                        for mb in temp:
                                            street['side01'].append(mb)
                                    ...
                                    temp = make_responce_of_two_point(
                                        str(point_one[sorted_points[start_point]]['lat']) + "," + str(
                                            point_one[sorted_points[start_point]]['long']),
                                        str(point_one[sorted_points[last_point]]['lat']) + "," + str(
                                            point_one[sorted_points[last_point]]['long']))
                                    for mb in temp:
                            ...
                            if dist <= 300:
                                ...
                                if last_point == -1:
                                    ...
                                    temp = make_responce_of_two_point(str(
                                        ...
                                        point_two[sorted_points[start_point]]['long']),
                                                                      str(
                                                                          point_two[sorted_points[last_point]][
                                                                              'lat']) + "," + str(
                                                                          point_two[sorted_points[last_point]][
                                                                              'long']))
                                    for mb in temp:
                ...
                if len(got_sec) == 2:
                    ...
                    temp = {
                        'start_latlng': str(street['violation'][0]['lat']) + "," + str(
                            street['violation'][0]['long']),
                        'end_latlng': "0,0"
                    }
                    street['side01'].append(temp)

            insert_map_view(str(street['street_id']), street['ref'], str(street['side01']), str(street['side02']),
                            str(street['total_violations']))
            ...
            for vio in point_one:
                ...
                for prev_vio in range(len(prev_point_one)):
                    ...
                    dist_A = calcCrow(vio['lat'], vio['long'],
                                      float(prev_point_one[prev_vio]['start_latlng'].split(',')[0]),
                                      float(prev_point_one[prev_vio]['start_latlng'].split(',')[1]))
                    if not prev_point_one[prev_vio]['end_latlng'] == "0,0":
                        dist_B = calcCrow(vio['lat'], vio['long'],
                                          float(prev_point_one[prev_vio]['end_latlng'].split(',')[0]),
                                          float(prev_point_one[prev_vio]['end_latlng'].split(',')[1]))
                        dist_AB = calcCrow(float(prev_point_one[prev_vio]['start_latlng'].split(',')[0]),
                                           float(prev_point_one[prev_vio]['start_latlng'].split(',')[1]),
                                           float(prev_point_one[prev_vio]['end_latlng'].split(',')[0]),
                                           float(prev_point_one[prev_vio]['end_latlng'].split(',')[1]))
                        ...
                        if dist_A <= 300 or dist_B <= 300:
                            ...
                            if dist_A >= dist_AB and dist_B <= 300:
                                ...
                                xx = make_responce_of_two_point_only(prev_point_one[prev_vio]['start_latlng'],
                                                                     prev_point_one[prev_vio]['end_latlng'])
                                if len(xx) > 0:
                                    prev_point_one[prev_vio]['end_latlng'] = str(vio['lat']) + "," + str(
                                        vio['long'])
                                    prev_point_one[prev_vio]['polylines'] = xx
                                    placed = True
                            elif dist_B >= dist_AB and dist_A <= 300:
                                xx = make_responce_of_two_point_only(
                                    prev_point_one[prev_vio]['start_latlng'],
                                    prev_point_one[prev_vio]['end_latlng'])
                                if len(xx) > 0:
                                    prev_point_one[prev_vio]['start_latlng'] = str(vio['lat']) + "," + str(
                                        vio['long'])
                                    prev_point_one[prev_vio]['polylines'] = xx
                                    placed = True
                    else:
            ...
            if len(not_placed_point_one) > 1:
                ...
                if max_distance <= 300:
                    temp = make_responce_of_two_point(str(
                        ...
                        not_placed_point_one[max_distance_point_01]['long']),
                                                      str(
                                                          not_placed_point_one[max_distance_point_02][
                                                              'lat']) + "," + str(
                                                          not_placed_point_one[max_distance_point_02]['long']))
                    for mb in temp:
                        prev_point_one.append(mb)
                            ...
                            if last_point == -1:
                                ...
                                temp = make_responce_of_two_point(str(
                                    ...
                                    not_placed_point_one[sorted_points[start_point]]['long']),
                                                                  str(
                                                                      not_placed_point_one[
                                                                          sorted_points[last_point]][
                                                                          'lat']) + "," + str(
                                                                      not_placed_point_one[
                                                                          sorted_points[last_point]]['long']))
                                for mb in temp:
                                    prev_point_one.append(mb)
                ...
                for prev_vio in range(len(prev_point_two)):
                    ...
                    if not prev_point_two[prev_vio]['end_latlng'] == "0,0":
                        ...
                        if dist_A <= 300 or dist_B <= 300:
                            ...
                            if dist_A >= dist_AB and dist_B <= 300:
                                xx = make_responce_of_two_point_only(
                                    prev_point_two[prev_vio]['start_latlng'],
                                    prev_point_two[prev_vio]['end_latlng'])
                                if len(xx) > 0:
                                    placed = True
                                    prev_point_two[prev_vio]['end_latlng'] = str(vio['lat']) + "," + str(
                                        vio['long'])
                                    prev_point_two[prev_vio]['polylines'] = xx
                                ...
                                if len(xx) > 0:
                                    placed = True
                                    prev_point_two[prev_vio]['start_latlng'] = str(vio['lat']) + "," + str(
                                        vio['long'])
                                    prev_point_two[prev_vio]['polylines'] = xx
                    else:
                        if dist_A <= 300:
            ...
            if len(not_placed_point_two) > 1:
                ...
                if max_distance <= 300:
                    temp = make_responce_of_two_point(str(
                        ...
                        not_placed_point_two[max_distance_point_01]['long']),
                                                      str(
                                                          not_placed_point_two[max_distance_point_02][
                                                              'lat']) + "," + str(
                                                          not_placed_point_two[max_distance_point_02]['long']))
                    for mb in temp:
                        prev_point_two.append(mb)
            ...
            update_map_view(str(street['street_id']), str(prev_point_one), str(prev_point_two),
                            str(total_violation))
            if len(set_of_street_ava_ids) > 0:
                update_street_status(set_of_street_ava_ids, 0)
    print(set_of_street_ava_ids)
...
@app.route('/upload_vio', methods=['POST'])
def upload_vio():
    req = request.json
    print("123")
    x = upload_dataset(req['vio'])
    print(x)
    return {'pending': x}

@app.route('/retry_upload_vio', methods=['POST'])
def retry_upload_vio():
    req = request.json
    x = retry_upload_dataset(req['vio'])
...
if __name__ == '__main__':
    app.run(debug=True, host="0.0.0.0", port=1255)
