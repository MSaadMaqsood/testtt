...
def get_prev_violation_last_3_day(str_id, udate, vio_type):
    ...
    query = ("SELECT `violation_id`,`violation_type_id`,`lat`,`long` FROM `violation` WHERE street_id='" +
             str(str_id)+"' AND violation_date BETWEEN '"+d.strftime('%Y-%m-%d')+"' AND '"+udate+"' AND `correct`='1' AND `violation_type_id`='"+str(vio_type)+"';")
    cursor.execute(query)
...
def get_duplicateViolation_forVioaltion(violationtype, violationDate, str_id, lat, lng):
    prev_violations = get_prev_violation_last_3_day(str_id, violationDate, violationtype)
    super_ids = "0"
    for j in prev_violations:
        dddd = calcCrow(float(lat), float(lng),float(j['lat']), float(j['lng']))
