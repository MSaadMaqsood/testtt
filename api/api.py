    ...
    try:
        ...
        if supers == "0":
            if correct == 1 and sensitivity == 1:
                ...
                cursor2 = cnx.cursor()
                query2 = (
                        "SELECT  `violation_type_id`, `display_img`, `lat`, `long`, `violation_date`, `violation_time` FROM `violation` WHERE `violation_id`='" + str(violation_id) + "';")
                cursor2.execute(query2)
                tempx = {'op_id': 0, 'display_img': '', 'lat': 0, 'long': 0, 'date': '', 'time': ''}
                for a, b, c, d, e, f in cursor2:
                    ...
                    tempx = {
                        ...
                        'date': e.strftime('%Y-%m-%d'), 'time': f.strftime('%H:%M')
                    }
                cursor2.close()
                apicallcombined_used(tempx)
...
def update_duplicate(violation_id, main_id, duplicate, user_id):
    ...
    try:
        if int(duplicate) == 0:
            ...
            cursor2 = cnx.cursor()
            query2 = (
                    "SELECT  `violation_type_id`, `display_img`, `lat`, `long`, `violation_date`, `violation_time` FROM `violation` WHERE `violation_id`='" + str(main_id) + "';")
            cursor2.execute(query2)
            tempx= {'op_id':0,'display_img':'','lat':0,'long':0,'date':'','time':''}
            for a, b, c, d, e, f in cursor2:
                img = cv2.imread('images/' + str(b))
                string_img = base64.b64encode(cv2.imencode('.jpg', img)[1]).decode()
                tempx = {
                    'op_id': int(main_id), 'display_img': string_img, 'lat': float(c), 'long': float(d), 'date': e.strftime('%Y-%m-%d'), 'time': f.strftime('%H:%M')
                }
            cursor2.close()
