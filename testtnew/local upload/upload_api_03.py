...
def get_prev_violation_last_3_day(str_id, udate):
    ...
    cnx = db_connection()
    cursor = cnx.cursor()
    query = ("SELECT `violation_id`,`violation_type_id`,`lat`,`long` FROM `violation` WHERE street_id='"+str(str_id)+"' AND violation_date BETWEEN '"+d.strftime('%Y-%m-%d')+"' AND '"+udate+"' AND correct='1';")
...
def main_function(list_of_all_violations, date_of_violation):
    ...
    for i in set_of_street_ava_ids:
        dict_of_Prev_violations_to_street[int(i)] = get_prev_violation_last_3_day(int(i), date_of_violation)
    for i in range(len(list_of_ava_violations)):
        if list_of_ava_violations[i]['street_id'] != 0 and int(list_of_ava_violations[i]['violation_type_id']) != 0:
            ...
            for j in dict_of_Prev_violations_to_street[int(list_of_ava_violations[i]['street_id'])]:
                ...
                if dddd < 100 and dddd<min_dist and int(list_of_ava_violations[i]['violation_type_id']) == int(j['type_id']):

                    list_of_ava_violations[i]['super_violation_id'] = int(j['vio_id'])
