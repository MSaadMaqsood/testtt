            ...
            for street in list_of_all_streets:
                if street['name'] == temp_name:
                    list_of_all_violations[vio]['street_id'] = int(
                        street['id'])
                    set_of_used_street_id.add(int(street['id']))
                    break
            if list_of_all_violations[vio]['street_id'] == 0:
                list_of_all_violations[vio]['street_id'] = upload_new_street_name(
                    temp_name)
                set_of_used_street_id.add(
                    int(list_of_all_violations[vio]['street_id']))
...
def side_check(lat1, lng1, lat2, lng2, test=0):
    url1 = "https://maps.googleapis.com/maps/api/directions/json?origin=" + \
        str(lat1) + "," + str(lng1) + "&destination=" + \
        str(lat2) + "," + str(lng2) + "&key=" + api_text
...
def make_sec_point(lat1, lng1, lat2, lng2,):
    ...
    url = "https://maps.googleapis.com/maps/api/directions/json?origin=" + \
        lat1+","+lng1+"&destination="+lat2+","+lng2+"&key=" + api_text
    response = requests.request("GET", url, headers={}, data={})
    re = []
    if len(response.json()['routes'][0]['legs'][0]['steps']) == 1:
...
def make_responce(latlng1, latlng2):
    ...
    url = "https://maps.googleapis.com/maps/api/directions/json?origin=" + \
        latlng1 + "&destination=" + latlng2 + "&key=" + api_text
    response = requests.request("GET", url, headers={}, data={})

    if len(response.json()['routes'][0]['legs'][0]['steps']) > 1:
        url1 = "https://maps.googleapis.com/maps/api/directions/json?origin=" + \
            latlng2 + "&destination=" + latlng1 + "&key=" + api_text
        response1 = requests.request("GET", url1, headers={}, data={})

        al = []

        l = polyline.decode(
            response1.json()["routes"][0]['overview_polyline']['points'])
        for i in l:
        ...
        al = []
        l = polyline.decode(
            response.json()["routes"][0]['overview_polyline']['points'])
        for i in l:
            temp = {'lat': i[0], 'lng': i[1]}
            al.append(temp)
        qwe = {
...
def new_side(temp, test=0):
    ...
    if len(temp) == 1:
        ...
        maxA = str(temp[max_distance_point_01]['lat'])+"," + \
            str(temp[max_distance_point_01]['lng'])
        maxB = str(temp[max_distance_point_02]['lat']) + \
            "," + str(temp[max_distance_point_02]['lng'])
        ...
        if max_distance <= 300:
            ...
            for count in range(len(temp) - 1):
                ...
                sorted_points_dist.append(
                    {'point': min_dist_point, 'dist': min_dis})
            ...
            for mnb in range(1, len(temp)):
                if sorted_points_dist[mnb]['dist']-sorted_points_dist[mnb-1]['dist'] > 300:
                    if start_point == mnb-1:
                        ...
                        z = get_second_latlng(
                            temp[sorted_points[start_point]]['lat'], temp[sorted_points[start_point]]['lng'])
                        if len(z) > 0:
                        ...
                        lines.append(make_responce(
                            str(temp[sorted_points[start_point]]['lat']) +
                            "," + str(temp[sorted_points[start_point]]['lng']),
...
def main_function(list_of_all_violations, date_of_violation):
    ...
    set_of_street_ava_ids = set_of_used_street_id.difference(
        set_of_unava_street_id)
    ...
    for violation in list_of_all_violations:
        if int(violation['street_id']) in set_of_unava_street_id:
            ...
            if int(violation['violation_type_id']) == 13:
                ...
                dict_of_trees_to_street[int(violation['street_id'])].append(
                    {'lat': violation['lat'], 'lng': violation['lng']})
            else:
                dict_of_violations_to_street[int(violation['street_id'])].append(
                    {'lat': violation['lat'], 'lng': violation['lng']})
    ...
    print("Available Streets: ", len(set_of_street_ava_ids))
    print("Unavailable Streets: ", len(set_of_unava_street_id))
    print("")
    print("Number of Trees: ", count_of_tree)
    print("Number of others violations: ", len(
        list_of_ava_violations)-count_of_tree)
    print("")
    print("Number of Unverified Violations: ", len(list_of_unava_violations))
    print("Number of Incorrect Violations: ",
          len(list_of_incorrect_violations))
    print(" ")
    ...
    # #########
    progress_counter = 0
    total_progress = len(set_of_street_ava_ids)*2
    sys.stdout.write("\r" + str(progress_counter)+"/"+str(total_progress))
    sys.stdout.flush()
    for street in {}:

        temp = dict_of_violations_to_street[street]
        ref_pointa, ref_pointb, prev_point_one, prev_point_two, total_violation = get_prev_violations_StreetID(
        ...
        if int(total_violation) > 0:
            ...
            for i in range(len(temp)):
                if side_check(temp[i]['lat'], temp[i]['lng'], ref_pointa['min'].split(',')[0], ref_pointa['min'].split(',')[1]):
                    side1.append(temp[i])
                elif side_check(temp[i]['lat'], temp[i]['lng'], ref_pointa['max'].split(',')[0], ref_pointa['max'].split(',')[1]):
                    side1.append(temp[i])
                else:
                    side2.append(temp[i])
            lines01 = update_side(side1, prev_point_one)
            lines02 = update_side(side2, prev_point_two)
            update_map_view(str(street), str(lines01), str(
                lines02), str(total_violation), date_of_violation)
        elif len(temp) > 0:
            if len(temp) == 1:
                lines = []
                z = get_second_latlng(temp[0]['lat'], temp[0]['lng'])
                if len(z) > 0:
                    ...
                    lines.append({'starting': str(temp[0]['lat']) + "," + str(
                        ...
                        'ending': '0,0',
                                  'polylines': []})
                insert_map_view(street, date_of_violation, {'min': str(z[0]['lat']) + "," + str(z[0]['lng']), 'max': str(z[0]['lat']) + "," + str(z[0]['lng'])},
                                {'min': 0, 'max': 0}, lines, [], 1)
            else:
                side1 = []
                side2 = []
        ...
        sys.stdout.write("\r" + str(progress_counter) +
                         "/" + str(total_progress))
        sys.stdout.flush()
    # Treeeeee
    ...
    for street in {}:
        ...
        if int(total_violation) > 0:
            ...
            lines01 = update_side(side1, prev_point_one)
            lines02 = update_side(side2, prev_point_two)
            update_map_view_tree(str(street), str(lines01), str(
                lines02), str(total_violation), date_of_violation)
        elif len(temp) > 0:
            if len(temp) == 1:
                lines = []
                ...
                if len(z) > 0:
                    ...
                    lines.append({'starting': str(temp[0]['lat']) + "," + str(
                        ...
                        'ending': '0,0',
                                  'polylines': []})
                insert_map_view_tree(street, date_of_violation, {'min': str(z[0]['lat']) + "," + str(z[0]['lng']), 'max': str(z[0]['lat']) + "," + str(z[0]['lng'])},
                                     {'min': 0, 'max': 0}, lines, [], 1)
            else:
                side1 = []
                side2 = []
    ...
    if len(list_of_incorrect_violations) > 0:
        insert_violations_Street(list_of_incorrect_violations)
    # if len(set_of_street_ava_ids) > 0:
