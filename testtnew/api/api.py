...
def inserttelemetryData():
    ...
    cnx = db_connection()
    cursor = cnx.cursor()
    query = ("INSERT INTO `violation`(`street_id`, `violation_type_id`, `details`, `accurate`, `risk`, `display_img`, `lat`, `long`, `start_latlng`, `end_latlng`, `violation_date`, `violation_time`, `violation_status`, `action_taken`, `user_id`) VALUES " +
             request_data['query'])
    cursor.execute(query)
    cnx.commit()
    cursor.close()
...
def get_violation_page(street_id):
    ...
    violation_table, violation_count = get_violation_table_now_default(
        street_id)
    return jsonify({
...
def get_street_health(street_id, violation_count):
    ...
    green_index = int((Asphalt+Sidewalk+Lighting+Cleanliness+Afforestation+Fossils +
                      Rubble_source+Street_Sweeping+Median+Communication_tower+Fly_Poster)/11)
    return {
...
def export_dashboard_csv():
    ...
    pdf_name = pdf_maker(x['street_name'], x['street_info'],
                         x['violation_date'], x['violation_table_data']['myData'])
...
def insertmapapi():
    ...
    cursor = cnx.cursor()
    query = ("INSERT INTO `map_api`(`api`, `comment`, `userid`, `entry_date`, `status`) VALUES ('" +
             str(request_data['api'])+"','"+str(request_data['comment'])+"','"+str(request_data['userid'])+"','"+today_date+"',1)")
    cursor.execute(query)
    cnx.commit()
    cursor.close()
...
@app.route('/update_violation_for_verify', methods=['GET', 'POST'])
def update_violation_for_verify():
    ...
    cursor1 = cnx.cursor()
    query1 = ("UPDATE `violation` SET `correct`='"+str(correct)+"', `violation_type_id`='"+str(updated_violation_type_id) +
              "', `street_id`='"+str(updated_street_id)+"' WHERE `violation_id`="+str(violation_id)+";")
    cursor1.execute(query1)
    cnx.commit()
    cursor1.close()
    ...
    if updated_violation_type_id != prev_violation_type_id and updated_street_id != prev_street_id:
        query = ("INSERT INTO `user_log`( `user_id`, `violation_id`, `prev_violation`, `updated_violation`, `prev_street`, `updated_street`, `correct_incorrect`, `entry_date`) VALUES ('"+str(user_id) +
                 "','"+str(violation_id)+"','"+str(prev_violation_type_id)+"','"+str(updated_violation_type_id)+"','"+str(prev_street_id)+"','"+str(updated_street_id)+"','"+str(correct)+"','"+today_date+"')")
